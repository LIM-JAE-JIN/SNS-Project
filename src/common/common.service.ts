...
export class CommonService {
  ...
  private composeFindOptions<T extends BaseModel>(
     ...
     * 나중에 where__likeCount__more_than 이나 where__title__ilike등 추가 필터를 넣고 싶을 때
     * 모든 where 필터들을 자동으로 파싱 할 수 있을만한 기능을 제작 해야한다.
     *
  ...
  private parseWhereFilter<T extends BaseModel>(
    ...
    /**
     * 예를 들어 where__id__more_than을
     * '__' 기준으로 나눴을 때
     *
     * ['where', 'id', 'more_than']으로 3개의 값으로 나눠진다.
     */
    ...
    if (split.length === 2) {
      ...
      /**
       * 길이가 3일 경우에는 Typorm 유틸리티 적용이 필요한 경우다.
       *
      ...
      // 해당 mapper가 단일값이 아닐 경우 if문으로 두개의 파라미터를 받을 수 있게 수정
      // if (operator === 'between') {
  ...
  //   if (split.length !== 2)
  //     throw new BadRequestException(
  //       `order 필터는 '__'로 split 했을 때, 길이가 2이어야 합니다. - 문제되는 값 : ${key}`,
  //     );
